 ...
 class BD {
    ...
    function __construct($s='127.0.0.1', $u='root', $p='', $b='') {
        ...
        if (!empty($b)) {
            ...
            $resp = self::$conn->query('SELECT database()');
            $resp = $resp->fetch_row();
            $this->base = $resp[0];
        }
    }
    ...
    function meta($s) {
     ...
     if (is_string($res)) {
      ...
      while ($col = $res->fetch_field()) {
       ...
       if (count($bt) === 2) {
        $resp = self::$conn->query("SHOW FULL columns FROM `$bt[0]`.`$bt[1]` LIKE '$nom'");
        if (!is_string($resp)) {
          ...
          $resp = self::$conn->query("SELECT CONCAT(REFERENCED_TABLE_SCHEMA,'.',referenced_table_name,'::',referenced_column_name) AS foranea FROM information_schema.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_SCHEMA = '$bt[0]' AND table_name = '$bt[1]' AND column_name = '$nom' AND REFERENCED_TABLE_NAME is not null");
          if ($resp->num_rows === 1) {
           $resp = $resp->fetch_assoc();
